  ...
  useEffect(() => {
    ...
    const xhr = doAjax({
      ...
      .done(function (data, textStatus, jqXHR) {
        if (jqXHR.imran) {
          console.log("aboring s");
          return;
        }
        console.log("credit checker ajax", data, jqXHR.imran);
        setState({ ...state, ...data.data, status: data.success ? 2 : 3 });
      })
      .fail(function (jqXHR, textStatus, errorThrown) {
        if (jqXHR.imran) {
          console.log("aboring f");
          return;
        }
        console.log(
          "credcheck ajax failed",
          textStatus,
          errorThrown,
          jqXHR.imran
        );
    ...
    return () => {
      console.log("unmounting credit checker");
      xhr.imran = 1;
    };
  }, []);
